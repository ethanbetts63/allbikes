...
def eligible_bikes():
    ...
    return Motorcycle.objects.filter(include_in_stock_alerts=True).prefetch_related('images').order_by('-date_posted', '-id')
...
def item_snapshot(bike, position):
    ...
    return {
        ...
        'deposit_url': f'{SITE_URL}/checkout/{bike.slug}?type=deposit',
    }


def _queue_items():
    return [item_snapshot(bike, position) for position, bike in enumerate(eligible_bikes(), start=1)]
...
def send_next_stock_alert():
    ...
    with transaction.atomic():
        ...
        items = [item_snapshot(bike, position) for position, bike in enumerate(bikes, start=1)]
        sent_count = 0
        failed_count = 0
