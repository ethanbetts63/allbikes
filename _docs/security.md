...
|---|---|---|---|---|---|
| `access_token` | JWT access token | Yes | Yes (prod) | Lax locally, None in prod | 60 minutes |
| `refresh_token` | JWT refresh token | Yes | Yes (prod) | Lax locally, None in prod | 30 days |
...
`SameSite` is `Lax` in local development. In production it is `None` with `Secure=True`, because the Vercel frontend and PythonAnywhere API are cross-site during deployment. `SameSite=None` means CSRF protection must come from the explicit CSRF token check described below rather than from SameSite cookie restrictions.
...
`POST /api/token/logout/` requires an authenticated request, then clears both cookies via `response.delete_cookie()`. The client state is also cleared. The call is best-effort from the UI's perspective: if it fails, the client state still resets.
...
The public frontend is served by Next.js on Vercel. Django is treated as the API/backend. Browser requests to `/api/*` go through the Next rewrite to the Django API, while public SEO routes such as `/sitemap.xml` and `/robots.txt` are generated by the Next app.

`frontend/proxy.ts` protects `/dashboard/:path*` before dashboard pages render. The proxy checks for either the access or refresh auth cookie and redirects unauthenticated users to `/login?next=/dashboard/...`. This is an early routing guard only; staff authorisation is still enforced by Django API permissions and by the dashboard's `/api/data/me/` check.

Dashboard, checkout, login, payment, processing, and confirmation routes are also marked noindex through route metadata or blocked in `robots.txt`. This is SEO hardening, not an access-control mechanism.
...
| `POST /api/token/logout/` | `IsAuthenticated` | Logout and clear auth cookies |
...
| `GET /api/hire/bookings/<reference>/` | `AllowAny` | Public reference lookup; does not return customer PII |
...
**Possible fix**: replace the public full-order lookup with a narrower confirmation endpoint that returns only the fields needed by the customer-facing success and processing pages, or require a second short-lived verification token alongside the reference.

### Hire booking retrieval is public but redacted

`GET /api/hire/bookings/{booking_reference}/` is `AllowAny`. It exists to power the hire payment and confirmation pages on refresh/direct-open. The endpoint intentionally does not return customer name, email, or phone. It returns booking reference, motorcycle, dates, extras, totals, and status.

The reference format is `HR-` + 8 random hex characters (~4 billion combinations). Brute forcing against the 250/day anonymous rate limit is not practical, and the response is redacted.
