    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      {models.length > 0 && (
        <section className="mb-8">
          <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-gray-500">Models</h2>
          <div className="flex flex-wrap gap-2">
            {models.map((m) => (
              <Link
                key={m.slug}
                href={`/parts/${m.slug}`}
                className="rounded-md border border-gray-200 bg-white px-3 py-2 text-sm hover:border-gray-400"
              >
                {m.name} <span className="text-gray-400">({m.model_code})</span>
              </Link>
            ))}
          </div>
        </section>
      ...
      {parts.length > 0 && (
        <section>
          <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-gray-500">Parts</h2>
    ...
    <div className="flex flex-wrap items-center justify-between gap-2 p-4">
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <span className="font-mono text-sm text-gray-800">{part.part_number}</span>
          {part.colour_name && (
            <span className="rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600">{part.colour_name}</span>
          )}
        </div>
      ...
      <div className="text-right">
        ...
        {target && <div className="text-xs text-gray-400">View diagram →</div>}
      </div>
    </div>
  );
