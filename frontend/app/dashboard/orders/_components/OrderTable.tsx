...
import { adminGetBikeOrders, adminGetProductOrders } from '@/lib/api';
...
const OrderTable = ({ kind }: { kind: 'product' | 'bike' }) => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        const result = kind === 'product'
          ? await adminGetProductOrders(statusParam, page)
          : await adminGetBikeOrders(statusParam, page);
  ...
  const columns: ColumnDef<Order>[] = useMemo(() => [
    ...
    },
    {
      ...
      accessorFn: (row) => row.order_kind === 'product' ? row.product_name : row.motorcycle_name,
      cell: ({ row }) => (
        <div className="text-[var(--text-dark-primary)]">
          <div>{row.original.order_kind === 'product' ? row.original.product_name : row.original.motorcycle_name}</div>
          {row.original.order_kind === 'bike' && row.original.selected_colour && (
            ...
            {table.getRowModel().rows.length ? (
              table.getRowModel().rows.map(row => (
                <TableRow
                  ...
                  onClick={() => router.push(`/dashboard/orders/${row.original.id}?kind=${row.original.order_kind}`)}
                >
