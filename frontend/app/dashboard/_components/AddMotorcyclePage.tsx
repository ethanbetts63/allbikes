...
import MotorcycleForm from "@/app/dashboard/_components/MotorcycleForm";
...
const AddMotorcyclePage = () => {
    ...
    const handleFormSubmit = async (data: MotorcycleFormData) => {
        ...
        try {
            ...
            const apiError = err as Error & { data?: Record<string, unknown> };
            let errorMessage: string;
            if (typeof apiError.data?.detail === 'string') {
                errorMessage = apiError.data.detail;
            } else if (apiError.data) {
                errorMessage = Object.entries(apiError.data)
